...
1. **Resolve input.** In order:
   ...
   - Else if a Codex plan or `/plan` block exists in the recent conversation, use it.
...
1. **Decompose into stories.** Each story should:
   ...
   - Be sized to roughly one worker agent's worth of work — bounded file set, single concern, runnable verify gate at the end.
