...
1. **Resolve spec.** In order:
   ...
   - Else check `docs/REWRITE_SPEC.md` → `docs/SPEC.md` → `README.md` → `AGENTS.md`.
...
3. **Detect existing `docs/ROADMAP.md`** (or `./ROADMAP.md` for spec-less projects). If present:
   - **`--update` flag passed** → enter update mode; read existing rows including their (number, title) identity pairs and any `<!-- auto -->` sentinel markers on Codex-drafted cells.
   - **No `--update` flag** → ask the user: *"Existing ROADMAP found. Fresh draft or in-place update?"* (one-shot multiple-choice prompt). Don't silently overwrite.
...
5. **Print the grouped list** and ask:
   ...
   Will execute:
     2. Per-section status walk — Codex proposes ✅/🟡/❌, PO confirms each
...
Coach: *"Sizing is for the PO, not for Codex. I'll propose; you correct. The point isn't precision — it's having a yardstick for trajectory and sprint scoping."*

For each section with status `❌` or `🟡`:

1. **Codex proposes a size** (S/M/L/XL) anchored on:
...
1. **Codex proposes dependency edges** from:
...
2. **Write the artifact with `<!-- auto -->` sentinel markers.**
   ...
   Every Codex-drafted cell gets a trailing `<!-- auto -->` HTML comment marker inside the table cell:

   ```
   | §A.6.2 popup chonks | 🟡 <!-- auto --> | M <!-- auto --> | itr#37 <!-- auto --> | chonks deferred to sprint-3 <!-- auto --> |
   ```

   The sentinel encodes a contract: **Codex owns the cell; `--update` will refresh it.**
   ...
   - Cells **without sentinel** → preserved verbatim (PO owns; Codex never overwrites).
   ...
   - **Multi-repo mode**: when `--scope` spans repos, write a single roadmap at the coordinating repo root (default current dir). Section rows declare repo prefix when the section lives elsewhere (e.g. `§B.1 sibling-repo:import flow`). One artifact per coordination point, not per repo — the roadmap is the cross-repo view.
