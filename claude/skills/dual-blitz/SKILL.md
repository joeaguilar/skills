---
name: dual-blitz
description: Plan and run a two-sided blitz by splitting backlog work into two isolated lane artifacts, one for each main agent, so each agent can run its own inner blitz without communicating or editing the same files. Trigger when the user types `/dual-blitz`, `/dual blitz`, "dual blitz agent 1", "dual blitz agent 2", asks to split a sprint or backlog between two simultaneous agents/sessions, or wants clustered blitz execution with stricter file-ownership boundaries than `/blitz`. Do NOT trigger for ordinary single-agent backlog execution — use `/blitz`. Do NOT trigger for planning-only sprint grooming — use `/sprint`. Do NOT trigger for the autonomous plan→execute→review loop — use `/overdrive`.
---

# /dual-blitz — two isolated blitz lanes

Split a sprint or backlog into two standalone execution lanes. Each lane is handed to a different main Claude Code session, and each main session may run its own inner `/blitz` waves. The outer split is stricter than `/blitz`: two main agents cannot coordinate, so a task may enter a lane only when its required writes are isolated from the other lane.
...
- For missing files, run one batched planner pass (a single `subagent_type: general-purpose` agent), not one subagent per task.
...
Use `AskUserQuestion` (or wait for an explicit reply) for the approval. If `--dry-run`, stop after printing this block.
