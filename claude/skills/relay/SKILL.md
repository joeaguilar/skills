---
name: relay
description: "Run ONE task as a chain of specialist stages — research → design → build → test — where each stage's output becomes the next stage's input, transforming the work forward to a final deliverable. Autonomous: infers the stages, runs them one at a time, hands the scroll forward. Trigger when the user types `/relay`, or asks to \"run this as a pipeline\", \"chain these stages\", \"do research then design then build then test\", \"pass this through stages\", \"set up a multi-stage handoff\", \"one stage feeds the next\", or \"sequential specialist pipeline\". Works for research→design→build→test and any dependent chain (`--write` for the build stages). Do NOT trigger to strike parallel facets of one task synthesized at once (that is a decomposition fan — there slices run together and merge; here stages run in order and transform), to make N independent attempts at the WHOLE task and keep the truest (that is a consensus swarm), or to loop a critic on ONE artifact polishing it pass after pass (that is a refine-loop). Relay = ordered, dependent, different-specialty stages."
---
