---
name: pre-mortem
description: "Assume ONE risky plan already failed and fan 3–7 subagents (`--axes=N`) over distinct failure axes to enumerate how it died, then rank failure modes by likelihood×impact and derive guardrails (`--apply` folds them into a hardened plan). Trigger: `/pre-mortem`, \"assume this failed and tell me why\", \"what could kill this\". NOT for forward planning/roadmapping — this works backward from failure."
---
