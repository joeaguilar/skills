---
name: overdrive
description: "Autonomous plan→execute→review loop. Condenses /sprint, /blitz, /sprint-review into one hands-off swarm. Pre-plans an implementation + exact owned-file set for EVERY ticket (the \"arm\" model), runs conflict-free waves of parallel agents on the SAME branch, commits once per wave so any wave rolls back to a clean state. ONLY human touch is a per-wave visual smoke test (accept → next wave; reject → roll back + iterate); planning, file assignment, execution, verify gates, quarantine triage, and review are all autonomous. Loops until every ticket is closed or quarantined. Trigger on `/overdrive`, or \"run the whole sprint autonomously\", \"plan execute and review end to end\", \"clear the backlog until every ticket is closed\", \"swarm the backlog hands-off\", \"autonomous sprint\", \"overdrive the backlog\", similar. `--auto`/`--trust` collapses the per-wave gate to one end-of-run review. Do NOT trigger for COACHED single-phase work: `/sprint` to plan WITH approval gates, `/blitz` for execution-only on an existing backlog WITHOUT the loop/commits, `/sprint-review` for review-only, `itr` skill for one-off filing. To review the plan before agents run, use `/sprint` (coached) or `/overdrive --dry-run`."
---
