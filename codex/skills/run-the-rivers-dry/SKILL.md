...
This skill does not override system instructions, tool permissions, repository instructions, safety constraints, budgets, or the user's right to stop or redirect the run.
...
`--god` selects the highest-effort orchestration profile. `--mortal` disables the chronicle voice and uses normal Codex prose. If both are present, keep the `--god` work profile and use mortal language. All flags are advisory unless the active environment and user instructions allow them.
...
- Auto-spawn read-only scout agents for broad or unclear problems when subagents are available.
...
2. **Think pass.** Build a brief risk map: dependencies, likely failure modes, unknowns, owned files, verification surfaces, and rollback concerns. Do not expose hidden chain-of-thought; summarize decisions and assumptions.
3. **Scout.** Spawn read-only explorer agents for independent unknowns when the problem is broad and the user has authorized this skill's agent behavior. Keep each scout prompt concrete and bounded.
...
5. **Swarm.** Spawn worker agents in parallel only when their write scopes do not conflict. Tell every worker they are not alone in the codebase, must not revert others' work, and must report changed files and verification.
...
- On simultaneous identical failures across a wave (API 429/overloaded, or workers completing without the expected structured result): treat it as a rate-limit cascade, not slice failure. Pause about 60 seconds, halve concurrency, and respawn only the failed workers without charging their retry budget.
