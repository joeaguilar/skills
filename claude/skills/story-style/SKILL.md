---
name: story-style
description: Build or update `STORY_STYLE.md` — the project-level convention file that `/sprint` reads in Phase 0 to mirror how this team writes issues, tickets, and stories. Walks the user through ingestion (real examples from `itr` / `gh` / pasted samples / pure interview), shows observed patterns, then offers either a linear field-by-field refinement or a free-form chat that the skill summarizes. Trigger when the user types `/story-style`, or asks to "create a story style", "set up STORY_STYLE.md", "how should we write stories", "what's our story format", "capture our issue conventions", "document our ticket style", or similar phrasing. Also surfaced as a soft suggest by `/sprint` Phase 0 when no `STORY_STYLE.md` exists. Setup wizard only — does NOT file issues, does NOT plan sprints, does NOT define a project-wide Definition of Done (style only; DoD stays per-sprint in `/sprint`).
---
...
Print a structured `Observed:` block (easier for the next agent to parse than prose):
...
2. **Ask the user to pick refinement mode** with `AskUserQuestion`:
...
Walk the field list below in order. Each field uses `AskUserQuestion` (multi-choice when there's a clear set of options) or short freeform (when the answer is project-specific text). The base default (or inferred value from Phase 3) is always offered as the **Recommended** option.
...
7. **AC observability rule** — how strict ("must be agent-checkable" vs "human-judged OK").
...
- **Teach the *why* of each schema section** in one short line as you reach it. Examples:
  - *"Title shape matters because every agent picking up an issue scans titles first; consistency speeds triage."*
  - *"AC observability is what lets an agent self-verify it's done — vague AC creates handoff failures."*
