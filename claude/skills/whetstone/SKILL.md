---
name: whetstone
description: "Take ONE artifact and grind it sharp over passes — draft → critique → revise → critique → revise — until it meets a quality bar, stops improving, or the passes run out. Cooperative, not adversarial: the critic's job is to make it BETTER, not to kill it. Autonomous: infers the bar, runs the loop without a gate, keeps the best version every pass, never ships a regression. Trigger when the user types `/whetstone`, or asks to \"polish this\", \"refine this over a few passes\", \"iterate on this draft\", \"make this sharper\", \"tighten this prose/design/spec\", \"critique-and-revise this\", \"grind this to a higher bar\", or \"keep improving this until it's good\". Works on prose, designs, functions, specs, and code (`--write`). Do NOT trigger for adversarial verification where a critic tries to BREAK or refute the thing and a judge rules whether it survives (that is an adversarial-duel job — there the critic kills, here it helps), for one independent attempt with no iteration, or for many parallel attempts on the same task voted into a consensus (that is a redundant-swarm job). Whetstone = cooperative, iterative, single-artifact polishing."
---
