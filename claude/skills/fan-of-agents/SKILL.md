---
name: fan-of-agents
description: "Take ONE task, cut it into aspects, throw 2–5 subagents (default 5, `--agents=N`) at it — each knife aimed at a different vital point — then synthesize whatever lands into one deliverable. Autonomous by design: infers the cut, fires without a gate, tolerates misses (not every knife hits; one or two is enough), never babysits failures, runs straight to synthesis. Trigger when the user types `/fan-of-agents`, or asks to \"fan out agents on this\", \"split this task across agents\", \"divide and conquer with subagents\", \"have N agents each take an aspect\", \"parallelize this one task across agents\", \"decompose and fan out\", \"throw a swarm at this\", or \"run a multi-perspective agent pass\". Works for research, analysis, design, audits, and implementation (`--write`). Do NOT trigger to clear an existing multi-task backlog in parallel (that is a backlog-blitz job keyed on file ownership across many pre-filed tasks), for serial single-aspect work one agent handles, or when the user wants a plan without execution."
---
