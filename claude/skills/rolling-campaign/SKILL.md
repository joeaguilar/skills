...
description: Run a chain of roadmap-bounded proof campaigns back-to-back, each in a FRESH cleared context, driven by an external headless loop (`scripts/chain.sh`) that relaunches the campaign per leg and halts safely on a wall. Every leg proves its work (tests, evidence, screenshots, full verify gate), writes a machine-readable `handoff.json` with an honest continue/halt decision, then a new process resumes the drafted next campaign with zero in-context dependency on the last. Trigger when the user types `/rolling-campaign`, `/chain-campaign`, asks to "chain campaigns", "run campaigns back-to-back", "clear context between campaigns and keep going", "keep running campaigns until the roadmap is done", "autonomous campaign chain", or "rolling/continuous/perpetual campaign". Use when the work outlasts a single orchestrator context window and must survive context resets. Do NOT trigger for a SINGLE self-contained campaign that fits one context (use `/proof-campaign`), for planning only (use `/sprint` or `/roadmap`), for executing one existing backlog (use `/blitz`), or for the in-context plan→execute→review loop (use `/overdrive`).
---
