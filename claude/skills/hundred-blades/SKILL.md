---
name: hundred-blades
description: "Attack the WHOLE task with N independent agents (default 5, `--blades=N`), each solving it from scratch, then keep the truest answer by vote, best-pick, or merge (`--write` applies the winner). Trigger: `/hundred-blades`, \"best of N\", \"self-consistency\", \"run it a few times and pick the best\". NOT to split into different aspects (use /fan-of-agents) or race for the first finisher (use /first-blood)."
---
