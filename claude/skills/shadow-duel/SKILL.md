---
name: shadow-duel
description: Verify or harden ONE claim, design, fix, or artifact by adversarial combat — a proposer defends it, challenger agents try to break it, a judge keeps only what survives, looping until it holds or falls. Autonomous: infers what's under test, fires without a gate, runs the duel to a verdict. Trigger when the user types `/shadow-duel`, or asks to "red-team this", "try to break this design", "adversarially verify", "stress-test this agent-vs-agent", "find the holes in this", "prove this is wrong", or "have agents argue this out". Works on claims, designs, plans, proofs, and code fixes (`--write`). Do NOT trigger to stress-test a plan by interviewing the human (that is a coached human-in-the-loop alignment job), to cooperatively polish your own draft pass after pass (that is a refine-loop / whetstone job — the critic there helps, here it kills), or for a single non-adversarial code review.
---
