...
tools: Glob, Grep, LS, Read, NotebookRead, WebFetch, TodoWrite, WebSearch, mcp__context7__get-library-docs, mcp__context7__resolve-library-id, Bash
color: yellow
---
...
2. **Performance**: Default to the React Compiler (React 19+) for automatic memoization — do NOT hand-wrap everything in `useMemo`/`useCallback`/`React.memo`. Since the compiler shipped, blanket manual memoization is itself an anti-pattern (noise, stale-dep bugs, fights the compiler). Reach for manual memoization only with a *measured* reason: a profiled re-render hot path, or referential stability a dependency array / context value genuinely needs. Flag both unmeasured manual memoization AND real, profiled performance regressions.
