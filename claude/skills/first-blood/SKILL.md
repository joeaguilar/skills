---
name: first-blood
description: Take ONE task, loose N RIVAL STRATEGIES at it in parallel (default 3, `--runners=N`) — each runner attacks the whole task by a different approach — and the FIRST to clear the bar wins; the rest are abandoned. Autonomous: infers the bar, fires without a gate, takes whoever lands first, vanishes the losers. Trigger when the user types `/first-blood`, or asks to "race a few approaches", "try several strategies in parallel and take the first that works", "fastest-correct wins", "speculative race", "fire multiple attempts and grab the first to clear the bar", "I don't know which approach works — run them all and take the winner", or "first past the post". Works for answers, designs, fixes, and code (`--write` lands the winner). Do NOT trigger to run N attempts and WAIT for all of them to vote for the truest answer (that is a consensus / variance-reduction job — there you wait for all and tally; here you take the first past the bar and cancel the rest), to escalate cheapest-approach-first SERIALLY and only try the next on failure (that is a cost-escalation ladder — there it's one-at-a-time; here every rival runs at once), or to split ONE task into different ASPECTS each agent owns a slice of (that is a decomposition fan — there each owns a facet; here every runner gets the whole task by a different strategy).
---
