...
- **Council seats** — 3 fresh contexts per premise decision, `codex exec -m gpt-5.5` via a thin wrapper agent (the `ambiguous` role in MODELS.md — Opus 5 is not effective at judging its own work); evidence seat MAY run gpt-5.6-sol for cross-model diversity when the Codex plugin is authenticated.
