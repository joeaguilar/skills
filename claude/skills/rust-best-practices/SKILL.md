...
description: Apply idiomatic Rust engineering standards when implementing, reviewing, or refactoring Rust code or Cargo projects — ownership/borrowing/lifetimes, error handling (anyhow/thiserror), public-API & trait design, async/Tokio concurrency, performance & allocation choices, testing strategy, Clippy lint setup, CI quality gates, and unsafe review. Routes to a detailed standards reference. Trigger when the user types /rust-best-practices, or asks to "review this Rust", "make this Rust idiomatic", "check my Cargo project against best practices", "help with this borrow-checker/lifetime error", or wants Rust ownership/async/error-handling guidance. Do NOT trigger for non-Rust code, for C++ standards (use /cpp-review), or for a generic cross-language diff review (use /code-review).
---
