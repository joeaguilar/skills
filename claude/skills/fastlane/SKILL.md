...
- Choose `/crossfire-blitz` over `/blitz` when the same prepared backlog would benefit from per-task model routing — a mix of mechanical work (cheap on gpt-5.5) and taste-critical work (opus-5) — and the user wants an independent cross-model review before each close. It needs the codex plugin authenticated for the Codex lanes; if Codex is unavailable, or the backlog is uniform mechanical work with no taste-sensitive surface, fall back to `/blitz`.
