...
description: "Clear a backlog with a model-routed parallel blitz: each task goes to the cheapest model that clears its bar (gpt-5.5 via Codex for bulk/generalist work; opus/fable for taste-critical), is cross-reviewed by a DIFFERENT model before close, and escalates when a cheaper model misses. Trigger: `/crossfire-blitz`, \"multi-model blitz\", \"route tasks to the cheapest capable model\". NOT for a single-model blitz (use /blitz)."
---

# crossfire-blitz — model-routed parallel backlog clearance

A blitz where **the model is chosen per task, not per session.** Bulk/mechanical work goes to **gpt-5.5** (the Codex generalist and agents default — cheapest, intelligence 8), with **gpt-5.6-terra** as the first escalation rung; user-facing / taste-critical work goes to **opus-5** or **fable-5**; every closed task is **cross-reviewed by a different model** than executed it. When a cheaper model's output misses the bar, the task is **redone by a smarter model without asking** — cost is a tie-breaker, never a reason to ship mediocre work.
...
| gpt-5.5 | 9 | 8 | 5 | Codex — `codex exec -m gpt-5.5` (pass `-m` explicitly; the `~/.codex/config.toml` default may differ). **The default Codex generalist / bulk workhorse — the agents default.** |
| sonnet-5 | 5 | 5 | 7 | Agent/Workflow `model: 'sonnet'` |
| opus-5 | 7 | 7 | 8 | Agent/Workflow `model: 'opus'` — the taste rung; **not effective at judging its own work** (never reviews its own output). |
| fable-5 | 2 | 9 | 9 | Agent/Workflow `model: 'fable'` |

**Executor assignment — classify each task, then route:**

1. **User-facing / taste-critical** — the task produces something a human sees or consumes (UI, UX, copy, error messages, public API/interface shape, docs). Taste must be **> 7** → **opus-5** (the default taste executor). **fable-5** is used only when `fable=on` *and* the surface is hero/flagship (or the task explicitly requested it — see the Fable gate). *Never a Codex model (terra/sol/luna/gpt-5.5) or sonnet-5 for the taste call — none clears the taste bar.*
2. **Bulk / mechanical** — clear-spec implementation, migrations, data transforms, mechanical refactors, backend logic with unambiguous acceptance → **gpt-5.5** (the generalist and agents default; intelligence 8; cheapest). **gpt-5.6-terra** is the first escalation rung, not the default.
3. **Ambiguous / judgment-heavy but not user-facing** — needs real reasoning, cross-cutting, spec not airtight → **gpt-5.5** (the `ambiguous` role; intelligence 8). Not opus-5: it sits at intelligence 7 and is not effective at judging its own work; opus-5 stays the taste executor only.
...
- Codex-executed task (gpt-5.5 et al.) → reviewed by a **different Codex model** (gpt-5.5 work → gpt-5.6-terra via the `/codex:adversarial-review` runtime; terra/sol work → another Codex rung), plus **fable-5** when `fable=on` for taste surfaces. Not opus-5 — reviews are rebound to Codex (Opus 5 is not effective at judging its own work, and at intelligence 7 it sits below every Codex rung).
...
**Escalation ladder** (standing permission for the first three rungs — no need to ask): a task whose review surfaces a real (P0/P1) defect is **redone by the next-smarter model**: gpt-5.5 → **gpt-5.6-terra** → **gpt-5.6-sol** → **fable-5** (opus-5 is not a non-taste rung — intelligence 7). The **fable rung is gated**: under `fable=off`, non-taste escalation tops out at gpt-5.6-sol (taste work at opus-5). But **topping out is not the same as silently redoing at the same model.** When a task was *already executed at the ceiling* (gpt-5.6-sol — or opus-5 for taste work — under `fable=off`) and its review still fails, "escalate to next-smarter" has no smarter rung left — the only same-tier move is **redo-by-same-model with the review findings spliced into the prompt**. Do that redo **once**; if it still fails, **do not silence the ceiling** — surface it to the user (or raise the Fable-gate `AskUserQuestion` for that task) and recommend one of the two real escalations above the ceiling: **enable fable-5** for the task, or **route it to `gpt-5.6-sol` at ultra** (`codex exec -m gpt-5.6-sol -c model_reasoning_effort="ultra"` — ultra is user-gated: run it only after the user picks this option, and as a **solo** subagent lane with nothing else in flight), which is also acceptable as a ceiling-breaker. Never quarantine a ceiling-executed task as "escalation exhausted" without first naming those two options — `fable=off` recommends against spending fable unattended, it does not forbid *recommending* it. Under `fable=on`, escalate straight through to fable with no question. Judge the output, not the price tag — escalating cost beats shipping the miss, *within the fable budget the user set.*

> **Codex reasoning-effort gotcha (bake this in).** Every `codex exec` in this skill omits an effort override by default, so it inherits `~/.codex/config.toml`'s `model_reasoning_effort` — which on some machines is set to `ultra`. The API **rejects `ultra`/`max` for gpt-5.5** (`Supported: none…xhigh`), so a `codex exec -m gpt-5.5` that inherits an `ultra` config **hard-fails on the very first task and blocks the whole Codex lane** — and gpt-5.5 is now the default lane. Fix: pin the effort explicitly on **every** `codex exec` invocation — implementation, worktree, and review — with `-c model_reasoning_effort="high"`. It is already baked into the canonical commands below; do not strip it.

> **Codex 5.6 effort & invocation rules (mirror of `claude/MODELS.md` — the canon).** **gpt-5.6-terra** runs at `medium`/`high`/`xhigh` (`high` is the standing default and what the canonical commands pin); **gpt-5.6-sol** runs at `medium`/`high`/`xhigh` (`medium` is its standing default). For both, **only use `ultra` if the user requested it, and only in a solo subagent run** — one Codex lane, nothing else in flight; never inside a parallel wave/fan-out. **gpt-5.6-luna is workflow-routed only**: it may take work an automated router assigned it (a `route:` tag, a cost-sensitive C1 batch) but is **never self-invoked** — don't pick luna on your own initiative, and it has no ultra lane (work that seems to need luna-at-ultra routes to terra/sol instead). On a **risky** task — miss is costly, spec subtle, blast radius wide — never run a Codex model below `high` and never route it to luna; if a risky task seems to demand `ultra` mid-wave, don't sneak it in — **defer it to `sonnet`** (a Claude executor), or surface the ultra request to the user and run it as a solo lane. Low/medium Codex effort is for cheap, unambiguous, mechanical work only; a risky task run cheap is how a subtle miss ships.
...
Requires: git repo(s); the tracker on PATH; whenever any task routes to a Codex model (gpt-5.6-terra et al.), the **codex plugin** installed and authenticated (`/codex:setup` fixes it — it wraps the Codex CLI, whose `~/.codex/config.toml` default may differ from the rubric's codex-default (gpt-5.5) — always select the executor model explicitly with `codex exec -m <model>`) so the `codex:codex-rescue` subagent and the `/codex:adversarial-review` runtime are reachable.
...
Crossfire-blitz preflight
  ...
  Concurrency:  <N>
  Repos:        <paths>
  Codex:        available (default exec model: gpt-5.5 — passed via -m; config default may differ) | UNAVAILABLE — Claude-only fallback
...
Codex can exceed Bash's 10-min window — handle that per the **completion contract** below (explicit `timeout`, harness-tracked backgrounding, sentinel file — never a sleep-poll). Add `--skip-git-repo-check` only if the target isn't a git repo. Use `--output-schema "$SCHEMA"` + `schema` on the wrapper when you want a validated machine-readable return.

> **The Codex completion contract (a known stall lives here — every rule is load-bearing).** Observed failure mode, repeatedly: the orchestrator parks "waiting for Codex", Codex finishes on disk, and the run never resumes. Every Codex lane — executor *and* reviewer — obeys four rules:
>
> 1. **Stay harness-tracked.** The only completion signal that reliably wakes the orchestrator is the harness's own `<task-notification>` for something *it* is running — the wrapper agent, or an orchestrator-level background Bash. Never dispatch a wave's Codex work through `codex:codex-rescue` or a detached `codex-companion.mjs task`: the rescue agent forwards once and returns (it is barred from polling), the companion job completes silently in its own store, and **no notification ever fires** — the classic "Codex completed but nobody was told" stall (observed).
> 2. **Never detach the codex call from the harness.** The wrapper runs `codex exec` in the **foreground** with an explicit Bash `timeout` (max 600000 ms); a task expected to exceed 10 minutes uses the Bash tool's `run_in_background: true` so the command's exit re-invokes its owner — never a shell `&`/`nohup` detach, which lets the wrapper report "done" while Codex still runs.
> 3. **Write a durable sentinel.** End every Codex lane's command with an on-disk marker next to the wave log: `codex exec … ; echo "exit=$?" > <wavelog-dir>/task-{id}.done`. Completion must be verifiable with `ls`/`cat` even if a notification is lost — the wave gate checks the sentinel, never lane silence, before treating a lane as finished.
> 4. **Waiting is active, never parked.** No foreground sleep-poll loops — an observed `for … do sleep 10` waiter died at Bash's 2-min default timeout (exit 143) and learned nothing. When a Codex lane's `<task-notification>` arrives, **harvest it immediately and continue the phase** — never answer a completion notification with "No response requested" (observed: a completed Codex lane's notification was dropped exactly that way and the run sat dead until unrelated events poked it). If a lane exceeds its deadline (default 15 min, or 2× its estimate), probe it actively — `pgrep -f "codex exec"`, the sentinel, the output tail — then harvest, retry once, or quarantine. Never end a turn "waiting for Codex" unless a live harness-tracked waiter exists.
...
- **P0/P1 finding** → **escalate**: redo the task with the next-smarter model (gpt-5.6-terra → gpt-5.6-sol → opus-5 → fable-5), splicing the review findings into the new executor's prompt. Log under `Escalations`. Re-review the redo (still cross-model). Two escalations without a clean review → **quarantine** (Phase 7) — **except** when the task was already at the ceiling (opus-5 under `fable=off`): there the "escalation" was a same-model redo-with-findings, so before quarantining, surface the ceiling and recommend fable-5 or `gpt-5.6-sol` at ultra (see the escalation ladder). Don't silently quarantine a ceiling task as if the ladder were truly exhausted.
- Record every review verdict under `Cross-model review` in the wave log with source attribution (which model reviewed).

Reviewer lanes obey the same **Codex completion contract** as executor lanes (Phase 4): harness-tracked dispatch, sentinel file, active waiting — a review lane that finishes unnoticed stalls the close exactly like an executor lane.
...
- Don't route user-facing / taste-critical work (taste must be > 7) to a Codex generalist (terra/sol/luna/gpt-5.5) or sonnet-5 — that's what opus-5 and fable-5 are for.
- Don't use Haiku for anything — executor, reviewer, wrapper, or planner.
- Don't spend fable-5 under `fable=off` — route taste work to opus-5, stop the escalation ladder at opus, and when a task requests fable surface the `AskUserQuestion` (with **off** as the first/default option) instead of silently enabling it. Never default that question to "on" or auto-answer it "on" in a headless run — off-first exists precisely so unattended runs never spend fable by accident.
...
- Don't dispatch any wave lane through `codex:codex-rescue` or a detached `codex-companion.mjs task` — the job completes silently in its own store and **nothing ever notifies the orchestrator** (the known mid-run stall). Every Codex lane stays harness-tracked per the Phase 4 completion contract and writes its `.done` sentinel.
- Don't park "waiting for Codex": no foreground sleep-poll loops, and never answer a Codex lane's completion `<task-notification>` with "No response requested" — harvest it and continue the phase. A turn may end waiting only while a live harness-tracked waiter exists, and every lane has a deadline after which it is actively probed (`pgrep`, sentinel, output tail).
...
- Don't run any Codex 5.6 model at `ultra` unless the user requested it **and** it runs as a solo subagent lane (one Codex lane, nothing else in flight — never mid-wave); terra and sol live at `medium`/`high`/`xhigh`. Don't self-invoke gpt-5.6-luna — it takes workflow-routed work only, never risky work, and has no ultra lane. Don't run a risky task below `high` on a Codex model — defer it to `sonnet` instead; low/medium Codex effort is for cheap mechanical work only.
