...
- `--tag <label>` — name the gate (`ci`, `test`, `clippy`). Everything
  ...
  command. Always tag standard gates.
...
the schema version; the `.meta.json` sidecar files next to each log carry the
same shape for daemon-style consumers.

## Config (`.gatr.toml` at repo root, optional)

```toml
[run]
filters = ["NumPy version"]
...
- **No live output.** The summary prints on completion; during a long build
  gatr is silent while the log is being written. For gates that can hang, add
  `--timeout`.
